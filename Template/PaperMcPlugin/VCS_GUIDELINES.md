# Version Control System (VCS) Guidelines / 版本控制規範 (通用模板)

此文件定義了專案的版本控制工作流程、分支模型與提交訊息標準，以確保協作效率與版本歷史的清晰度。

## 1. Branching Model / 分支模型

本專案建議採用 **GitFlow** 或簡化的 **GitHub Flow** 作為核心分支策略：
- **`main` (或 `master`)**: 用於存放穩定、可隨時發布的正式版本。
- **`develop`** (選用): 開發主分支，整合所有已完成的功能。此分支應保持相對穩定，是新功能分支的基礎。
- **`feature/<feature-name>`**: 用於開發新功能的分支。
  - 命名規則: `feature/` 前綴，後面跟隨簡短且描述性的功能名稱（例如 `feature/player-stats`）。
...
- **`<type>`**: 提交類型，必須是以下之一：
  ...
  - `perf`: 提升效能的程式碼變更
  - `test`: 新增或修改測試
  - `chore`: 維護性工作，如更新建置腳本、套件、重構專案結構等
- **`<scope>` (可選)**: 影響範圍，例如模組名稱 (e.g., `database`, `core`, `module-name`)。
- **`<subject>`**: 簡潔描述，使用祈使句，首字母小寫，結尾不加句號。

**範例**:
- `feat(database): implement hikaricp connection pool`
- `fix(command): resolve null pointer exception in give command`
- `docs(readme): update build instructions`
- `refactor(core): abstract player data loading logic`
...
- **審查**: 在合併前，至少需要一位團隊成員的審查批准 (Review Approve) 或確保自動化測試通過。